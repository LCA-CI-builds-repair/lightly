...
def _check_matplotlib_available() -> None:
    if isinstance(plt, Exception):
        raise ImportError("Matplotlib is not available. Please install it.")
...
def std_of_l2_normalized(z: torch.Tensor) -> torch.Tensor:
    ...
    Args:
        z (torch.Tensor): A torch tensor of shape batch_size x dimension.
    
    Returns:
        torch.Tensor: The mean of the standard deviation of the l2 normalized tensor z along each dimension.
    """
    if len(z.shape) != 2:
...
def apply_transform_without_normalize(
    ...
    Args:
        image (Image.Image): Input image to be transformed.
        transform: Transformation to be applied excluding ToTensor and Normalize.
    
    Returns:
        Image.Image: Transformed image without ToTensor and Normalize.
    """
...
def generate_grid_of_augmented_images(
    ...
    Args:
        ...
        collate_function (Union[BaseCollateFunction, MultiViewCollateFunction]): Collate function for self-supervised learning.
    
    Returns:
        List[List[Image.Image]]: Grid of augmented images.
    """
    grid = []
