import torch
import torch.distributedimport torch
...
class SSLEYLoss(torch.nn.Module):
    ...
    def forward(self, z_a: torch.Tensor, z_b: torch.Tensor) -> torch.Tensor:
        ...
        assert (
            z_a.shape == z_b.shape
        ), f"z_a and z_b must have the same shape but found {z_a.shape} and {z_b.shape}."
        ...
        if self.gather_distributed and dist.is_initialized():
            ...
            if world_size > 1:
                z_a = torch.cat(dist.gather(z_a), dim=0)
                z_b = torch.cat(dist.gather(z_b), dim=0)
